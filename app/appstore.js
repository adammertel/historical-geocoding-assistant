  ...
  */
  @action findDefaultColumnNames () {
    const columns = Object.keys(this.recordData).map(c => {
      return {
        sanitized: c.toLowerCase().trim().replace(/_/g, '').replace(/-/g, ''),
        original: c
      }
    })

    const mandatoryColumnKeywords = {
      name: [
        'placename',
        'locationname',
        'location',
  ...
  @computed get mapPosition () {
    return this.opts.mapCenter ? [this.opts.mapCenter[0], this.opts.mapCenter[1]] : [0, 0]
  }
  ...
  @computed get recordData () {
    return this.records[this.row] ? Object.assign(this.records[this.row], {}) : {}
  }
  ...
  @action saveRecord = () => {
    ...
    Sheet.updateLine(this.row, Object.values(this.recordData), () => this.updateData())
  }
