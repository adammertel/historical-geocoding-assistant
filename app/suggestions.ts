...
import { SuggestionRecord, SuggestionSource } from "@types";
...
const checkNoLimit = <T>(suggestions: T[], no = 10): T[] => {
...
const _doFetch = (
  ...
  sourceNext: (data: SuggestionRecord[], problem: boolean) => void,
  next: (res: any) => void
): void => {
  Base.doFetch(url, opts, (err: any, res: any) => {
    if (err) {
      sourceNext([], true);
    } else if (!res) {
      sourceNext([], true);
...
const suggestionGeonames: SuggestionSource = {
  id: "geonames",
  label: "GeoNames",
  urls: {
    base: (term: string) =>
...
const suggestionNominatim: SuggestionSource = {
  id: "nominatim",
  label: "Nominatim",
  urls: {
    base: (term: string) =>
...
const suggestionWiki: SuggestionSource = {
  id: "wiki",
  label: "Wikipedia",
  urls: {
    base: (term: string) =>
...
const suggestionChina: SuggestionSource = {
  id: "tgaz",
...
const suggestionTGN: SuggestionSource = {
  id: "getty",
  label: "Getty TGN",
  urls: {
    base: (term: string) =>
    ...
    record: (id: string) => "http://vocab.getty.edu/tgn/" + id + "-geometry",
  },
  ...
  getRecords: (source, term, opts, next) => {
    ...
    _doFetch(url, {}, next, (res) => {
      ...
      results = resultNodes.map((result) => {
        const tds = Base.query(result, "td").map((t) => {
...
const suggestionPleiades: SuggestionSource = {
  id: "pleiades",
  label: "Pleiades",
  preload: () => {
    Base.doFetch(
      ...
      (err: any, res: any) => {
        if (res) {
          data["pleiades"] = res.filter(
            (r: any) => r[0] && r[0] !== "Untitled"
          );
        }
      }
    );
  ...
  getRecords: (source, term, opts, next) => {
    ...
    similars.forEach((similar) => {
      ...
      Base.doFetch(url, {}, (err: any, res: any) => {
        processed++;
